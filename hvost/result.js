...
async function fetchDataWithCache(sheetName = 'hvostRes', range = 'A1:L120') {
...
async function renderTable() {
    ...
    for (const range of RANGE_PARTS) {
        try {
            const data = await fetchDataWithCache('hvostRes', range);
            if (!data || !data.values) {
...
async function renderData(sheetName = 'hvostRes') {
    try {
