...
async function fetchDataWithCache(sheetName = 'brickusRes', range = 'A1:H650') {
...
async function renderTable() {
    ...
    for (const range of RANGE_PARTS) {
        try {
            const data = await fetchDataWithCache('brickusRes', range);
            if (!data || !data.values) {
...
async function renderData(sheetName = 'brickusRes') {
    try {
