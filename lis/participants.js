document.addEventListener('DOMContentLoaded', function () {
    ...
    function createSelect(id, dataColumn, dataRow, placeholder) {
        ...
        select.addEventListener('input', function () {
            saveData(select.value, dataColumn, dataRow, 'christinaDay2');
        });
...
function createCheckbox(id, dataColumn, dataRow, initialValue) {
    ...
    checkbox.addEventListener('change', function () {
        if (checkbox.checked) {
            saveData('Номинант', dataColumn, dataRow, 'christinaDay2');
        } else {
            saveData('', dataColumn, dataRow, 'christinaDay2');
        }
    });
...
function createInputFields(container, rowId, placeholders, options = []) {
 ...
 function createParticipantPanel(participant, placeholders) {
    ...
    async function fetchDataWithCache(sheetName = 'christinaDay2', includeParticipants = false) {
    ...
    async function renderData(sheetName = 'christinaDay2') {
...
async function saveData(value, column, row, sheetName = 'christinaDay2') {
...
function attachInputListeners() {
    ...
    textareas.forEach(textarea => {
        textarea.addEventListener('change', function () {
            saveData(this.value, this.getAttribute('data-column'), this.getAttribute('data-row'), 'christinaDay2');
        });
    });
}
