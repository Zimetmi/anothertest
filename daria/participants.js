document.addEventListener('DOMContentLoaded', function () {
    ...
    function createSelect(id, dataColumn, dataRow, placeholder) {
        ...
        select.addEventListener('input', function () {
            saveData(select.value, dataColumn, dataRow, 'daria');
        });
...
function createCheckbox(id, dataColumn, dataRow, initialValue) {
    ...
    checkbox.addEventListener('change', function () {
        if (checkbox.checked) {
            saveData('Номинант', dataColumn, dataRow, 'daria');
        } else {
            saveData('', dataColumn, dataRow, 'daria');
        }
    });
...
//        { label: 'Аксессуары', column: 'F', placeholder: placeholders['dropdown'] }
    ...
    saveData(this.value, 'F', rowId, 'daria');
	}, 300));  // Задержка 300 мс
 ...
 function createParticipantPanel(participant, placeholders) {
    ...
    async function fetchDataWithCache(sheetName = 'daria', includeParticipants = false) {
    ...
    async function renderData(sheetName = 'daria') {
...
function saveData(value, column, row, sheetName = 'daria') {
...
function attachInputListeners() {
    ...
    textareas.forEach(textarea => {
        textarea.addEventListener('change', function () {
            saveData(this.value, this.getAttribute('data-column'), this.getAttribute('data-row'), 'daria');
        });
    });
}
