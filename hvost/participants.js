document.addEventListener('DOMContentLoaded', function () {
    ...
    function createSelect(id, dataColumn, dataRow, placeholder) {
        ...
        select.addEventListener('input', function () {
            saveData(select.value, dataColumn, dataRow, 'hvost');
        });
...
function createCheckbox(id, dataColumn, dataRow, initialValue) {
    ...
    checkbox.addEventListener('change', function () {
        if (checkbox.checked) {
            saveData('Номинант', dataColumn, dataRow, 'hvost');
        } else {
            saveData('', dataColumn, dataRow, 'hvost');
        }
    });
...
//        { label: 'Аксессуары', column: 'F', placeholder: placeholders['dropdown'] }
    ...
    saveData(this.value, 'F', rowId, 'hvost');
	}, 300));  // Задержка 300 мс
 ...
 function createParticipantPanel(participant, placeholders) {
    ...
    async function fetchDataWithCache(sheetName = 'hvost', includeParticipants = false) {
    ...
    async function renderData(sheetName = 'hvost') {
...
function saveData(value, column, row, sheetName = 'hvost') {
...
function attachInputListeners() {
    ...
    textareas.forEach(textarea => {
        textarea.addEventListener('change', function () {
            saveData(this.value, this.getAttribute('data-column'), this.getAttribute('data-row'), 'hvost');
        });
    });
}
